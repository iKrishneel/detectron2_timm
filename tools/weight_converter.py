...
def swin_models(weights, model_name: str) -> OrderedDict:
    state_dict = weights['state_dict']
    new_state_dict = OrderedDict()
    ...
    for key in state_dict:
        new_key = None
        if 'backbone' in key:
            if 'backbone.layers' in key or 'backbone.patch_embed' in key:
                new_key = 'backbone.bottom_up.model.model.' + key[9:]
...
def main(args):
    ...
    if 'xcit' in name:
        ...
        new_state_dict = swin_models(weights, args.weights)
    else:
